...
class SystemPromptPart:
    ...
    Only set if system prompt is dynamic, see [`system_prompt`][liquid_os.Agent.system_prompt] for more information.
    """
...
class UserPromptPart:
    ...
    Content comes from the `user_prompt` parameter of [`Agent.run`][liquid_os.Agent.run],
    [`Agent.run_sync`][liquid_os.Agent.run_sync], and [`Agent.run_stream`][liquid_os.Agent.run_stream].
    """
...
tool_return_ta: pydantic.TypeAdapter[Any] = pydantic.TypeAdapter(
    Any, config=pydantic.ConfigDict(defer_build=True)
)
...
class RetryPromptPart:
    ...
    * a tool raised a [`ModelRetry`][liquid_os.exceptions.ModelRetry] exception
    * no tool was found for the tool name
    * the model returned plain text when a structured response was expected
    * Pydantic validation of a structured response failed, here content is derived from a Pydantic
      [`ValidationError`][pydantic_core.ValidationError]
    * a result validator raised a [`ModelRetry`][liquid_os.exceptions.ModelRetry] exception
    """
...
"""A message part sent by LiquidOS to a model."""


@dataclass
class ModelRequest:
    """A request generated by LiquidOS and sent to a model, e.g. a message from the LiquidOS app to the model."""
...
class ToolCallPart:
    ...
    @classmethod
    def from_raw_args(
        cls, tool_name: str, args: str | dict[str, Any], tool_call_id: str | None = None
    ) -> Self:
...
ModelResponsePart = Annotated[
    Union[TextPart, ToolCallPart], pydantic.Discriminator("part_kind")
]
...
@dataclass
class ModelResponse:
    """A response from a model, e.g. a message from the model to the LiquidOS app."""
...
ModelMessage = Annotated[
    Union[ModelRequest, ModelResponse], pydantic.Discriminator("kind")
]
"""Any message sent to or returned by a model."""

ModelMessagesTypeAdapter = pydantic.TypeAdapter(
    list[ModelMessage], config=pydantic.ConfigDict(defer_build=True)
)
    ...
    def _apply_to_delta(
        ...
        if self.tool_call_id:
            ...
            if (
                ...
                and delta.tool_call_id != self.tool_call_id
            ):
                raise UnexpectedModelBehavior(
...
ModelResponsePartDelta = Annotated[
    Union[TextPartDelta, ToolCallPartDelta], pydantic.Discriminator("part_delta_kind")
]
...
ModelResponseStreamEvent = Annotated[
    Union[PartStartEvent, PartDeltaEvent], pydantic.Discriminator("event_kind")
]
