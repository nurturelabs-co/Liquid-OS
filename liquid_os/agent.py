...
from typing import Any, Callable, Generic, Literal, Optional, cast, final, overload
...
_logfire = logfire_api.Logfire(otel_scope="liquid-os")
...
try:
    ...
    logfire._internal.stack_info.NON_USER_CODE_PREFIXES += (
...
class Agent(Generic[AgentDeps, ResultData]):
    ...
    Agents are generic in the dependency type they take [`AgentDeps`][liquid_os.tools.AgentDeps]
    and the result data type they return, [`ResultData`][liquid_os.result.ResultData].
    ...
    _result_schema: _result.ResultSchema[ResultData] | None = dataclasses.field(
        repr=False
    )
    ...
    _system_prompt_dynamic_functions: dict[
        str, _system_prompt.SystemPromptRunner[AgentDeps]
    ] = dataclasses.field(repr=False)
    _deps_type: type[AgentDeps] = dataclasses.field(repr=False)
    _max_result_retries: int = dataclasses.field(repr=False)
    _override_deps: _utils.Option[AgentDeps] = dataclasses.field(
        default=None, repr=False
    )
    _override_model: _utils.Option[models.Model] = dataclasses.field(
        default=None, repr=False
    )
        ...
        *,
        ...
        Args:
            ...
            system_prompt: Static system prompts to use for this agent, you can also register system
                prompts via a function with [`system_prompt`][liquid_os.Agent.system_prompt].
            ...
            tools: Tools to register with the agent, you can also register tools via the decorators
                [`@agent.tool`][liquid_os.Agent.tool] and [`@agent.tool_plain`][liquid_os.Agent.tool_plain].
            defer_model_check: by default, if you provide a [named][liquid_os.models.KnownModelName] model,
                it's evaluated to create a [`Model`][liquid_os.models.Model] instance immediately,
                which checks for the necessary environment variables. Set this to `false`
                to defer the evaluation until the first run. Useful if you want to
                [override the model][liquid_os.Agent.override] for testing.
            end_strategy: Strategy for handling tool calls that are requested alongside a final result.
                See [`EndStrategy`][liquid_os.agent.EndStrategy] for more information.
        """
        ...
        *,
        ...
        with _logfire.span(
            ...
            run_context = RunContext(
                deps, model_used, usage or _usage.Usage(), user_prompt
            )
            messages = await self._prepare_messages(
                user_prompt, message_history, run_context
            )
            ...
            while True:
                ...
                with _logfire.span(
                    "model request", run_step=run_context.run_step
                ) as model_req_span:
                    model_response, request_usage = await agent_model.request(
                ...
                with _logfire.span(
                    "handle model response", run_step=run_context.run_step
                ) as handle_span:
                    ...
                    if final_result is not None:
                        ...
                        tool_responses_str = " ".join(
                            r.part_kind for r in tool_responses
                        )
                        handle_span.message = (
                            f"handle model response -> {tool_responses_str}"
                        )
    ...
    def run_sync(
        ...
        *,
        ...
        This is a convenience method that wraps [`self.run`][liquid_os.Agent.run] with `loop.run_until_complete(...)`.
    ...
    def run_stream(
        ...
        *,
        ...
        result.StreamedRunResult[AgentDeps, RunResultData]
    ]: ...
        ...
        *,
        ...
        with _logfire.span(
            ...
            while True:
                ...
                with _logfire.span(
                    "model request {run_step=}", run_step=run_context.run_step
                ) as model_req_span:
                    async with agent_model.request_stream(
                        ...
                        model_req_span.set_attribute(
                            "response_type", model_response.__class__.__name__
                        )
                        ...
                        with _logfire.span("handle model response") as handle_span:
                            ...
                            if isinstance(maybe_final_result, _MarkFinalResult):
                                ...
                                async def on_complete():
                                    ...
                                    assert isinstance(
                                        last_message, _messages.ModelResponse
                                    )
                                    tool_calls = [
                                        part
                                ...
                                if tool_responses:
                                    ...
                                    messages.append(
                                        _messages.ModelRequest(tool_responses)
                                    )
    ...
    def system_prompt(
        ...
        *,
        ...
        Optionally takes [`RunContext`][liquid_os.tools.RunContext] as its only argument.
        ...
        Args:
            ...
            dynamic: If True, the system prompt will be reevaluated even when `messages_history` is provided,
                see [`SystemPromptPart.dynamic_ref`][liquid_os.messages.SystemPromptPart.dynamic_ref]
        ...
        if func is None:
            ...
            self._system_prompt_functions.append(
                _system_prompt.SystemPromptRunner(func, dynamic=dynamic)
            )
            return func
    ...
    def result_validator(
        self,
        func: Callable[[RunContext[AgentDeps], ResultData], Awaitable[ResultData]],
        /,
    ) -> Callable[[RunContext[AgentDeps], ResultData], Awaitable[ResultData]]: ...

    @overload
    def result_validator(
        self, func: Callable[[ResultData], ResultData], /
    ...
    def result_validator(
        ...
        Optionally takes [`RunContext`][liquid_os.tools.RunContext] as its first argument.
        Can decorate a sync or async functions.
        ...
        Example:
        ```python
        from liquid_os import Agent, ModelRetry, RunContext
    ...
    @overload
    def tool(
        self, func: ToolFuncContext[AgentDeps, ToolParams], /
    ...
    def tool(
        ...
        *,
        ...
        [ToolFuncContext[AgentDeps, ToolParams]], ToolFuncContext[AgentDeps, ToolParams]
    ]: ...
    ...
    def tool(
        ...
        *,
        ...
        """Decorator to register a tool function which takes [`RunContext`][liquid_os.tools.RunContext] as its first argument.
        ...
        Args:
            ...
            prepare: custom method to prepare the tool definition for each step, return `None` to omit this
                ...
                or omit it completely from a step. See [`ToolPrepareFunc`][liquid_os.tools.ToolPrepareFunc].
            docstring_format: The format of the docstring, see [`DocstringFormat`][liquid_os.tools.DocstringFormat].
    ...
    def _register_tool(self, tool: Tool[AgentDeps]) -> None:
        ...
        if tool.name in self._function_tools:
            raise exceptions.UserError(
                f"Tool name conflicts with existing tool: {tool.name!r}"
            )

        if self._result_schema and tool.name in self._result_schema.tools:
            raise exceptions.UserError(
                f"Tool name conflicts with result schema name: {tool.name!r}"
            )
    ...
    async def _prepare_model(
        ...
        async def add_tool(tool: Tool[AgentDeps]) -> None:
            ctx = run_context.replace_with(
                retry=tool.current_retry, tool_name=tool.name
            )
                ...
                if isinstance(msg, _messages.ModelRequest):
                    for i, part in enumerate(msg.parts):
                        if (
                            ...
                            and part.dynamic_ref
                        ):
                            # Look up the runner by its ref
                            if runner := self._system_prompt_dynamic_functions.get(
                                part.dynamic_ref
                            ):
    ...
    async def _prepare_messages(
        ...
        if message_history:
            ...
            if user_prompt:
                messages.append(
                    _messages.ModelRequest([_messages.UserPromptPart(user_prompt)])
                )
        else:
    ...
    async def _handle_model_response(
        ...
        _MarkFinalResult[RunResultData] | None, list[_messages.ModelRequestPart]
    ]:
        ...
        if tool_calls:
            return await self._handle_structured_response(
                tool_calls, run_context, result_schema
            )
        elif texts:
            text = "\n\n".join(texts)
    ...
    async def _handle_text_response(
        ...
        if self._allow_text_result(result_schema):
            ...
            try:
                result_data = await self._validate_result(
                    result_data_input, run_context, None
                )
    ...
    async def _process_function_tools(
        ...
        """Process all function (non-result) tool calls in parallel.

        Also handle client-side vs agent-side distinction:
          - if the tool is client_side=True, just return a special marker
            for the client to handle externally
          - otherwise, the agent runs it
        """
        parts: list[_messages.ModelRequestPart] = []
        ...
        for call in tool_calls:
            if call.tool_name == result_tool_name and not found_used_result_tool:
                ...
                if stub_function_tools:
                    ...
                    if tool.client_side:
                        parts.append(ClientSideToolCallReturnPart(tool_call=call))
                    else:
                        tasks.append(
                            asyncio.create_task(
                                tool.run(call, run_context), name=call.tool_name
                            )
                        )
                ...
                parts.append(
                    self._unknown_tool(call.tool_name, run_context, result_schema)
                )

        if tasks:
            with _logfire.span("running {tools=}", tools=[t.get_name() for t in tasks]):
                task_results: Sequence[
    ...
    async def _handle_streamed_response(
        ...
        _MarkFinalResult[models.StreamedResponse]
        ...
        async for maybe_part_event in streamed_response:
            if isinstance(maybe_part_event, _messages.PartStartEvent):
                ...
                if isinstance(new_part, _messages.TextPart):
                    ...
                    if result_schema is not None and (
                        match := result_schema.find_tool([new_part])
                    ):
                        call, _ = match
        ...
        for p in model_response.parts:
            if isinstance(p, _messages.ToolCallPart):
                if tool := self._function_tools.get(p.tool_name):
                    if tool.client_side:
                        # Skip execution, return the part
                        parts.append(ClientSideToolCallReturnPart(tool_call=p))
                    else:
                        tasks.append(
                            asyncio.create_task(
                                tool.run(p, run_context), name=p.tool_name
                            )
                        )
                else:
                    parts.append(
                        self._unknown_tool(p.tool_name, run_context, result_schema)
                    )

        if received_text and not tasks and not parts:
            # The model returned only text, but we do not allow text results
            self._incr_result_retry(run_context)
        ...
        with _logfire.span("running {tools=}", tools=[t.get_name() for t in tasks]):
            task_results: Sequence[_messages.ModelRequestPart] = await asyncio.gather(
                *tasks
            )
            parts.extend(task_results)
    ...
    async def _validate_result(
        ...
        if self._result_validators:
            ...
            for validator in self._result_validators:
                agent_result_data = await validator.validate(
                    agent_result_data, tool_call, run_context
                )
    ...
    async def _sys_parts(
        self, run_context: RunContext[AgentDeps]
    ) -> list[_messages.ModelRequestPart]:
        """Build the initial messages for the conversation."""
        messages: list[_messages.ModelRequestPart] = [
            _messages.SystemPromptPart(p) for p in self._system_prompts
        ]
            ...
            if sys_prompt_runner.dynamic:
                messages.append(
                    _messages.SystemPromptPart(
                        prompt, dynamic_ref=sys_prompt_runner.function.__qualname__
                    )
                )
            else:
...
def capture_run_messages() -> Iterator[list[_messages.ModelMessage]]:
    """Context manager to access the messages used in a [`run`][liquid_os.Agent.run], [`run_sync`][liquid_os.Agent.run_sync], or [`run_stream`][liquid_os.Agent.run_stream] call.

    Useful when a run may raise an exception, see [model errors](../agents.md#model-errors) for more information.

    Examples:
    ```python
    from liquid_os import Agent, capture_run_messages
...
@dataclasses.dataclass
class ClientSideToolCallReturnPart:
    """A special message part indicating that a tool call should be handled by the client.

    Includes the original `ToolCallPart` so the client can inspect `tool_name`, arguments, etc.
    """

    tool_call: _messages.ToolCallPart
    part_kind: Literal["client_side_tool_call_return"] = "client_side_tool_call_return"

    @property
    def content(self) -> str:
        return f"Client-side tool call: {self.tool_call.tool_name}"
    ...
    def dict(self) -> dict[str, Any]:
        return {
            ...
            "arguments": self.tool_call.payload,
        }
...
class ToolCallRequest(BaseModel):
    ...
    toolCallId: str
    toolName: str
    args: _messages.ArgsJson | _messages.ArgsDict
    state: Optional[str] = None
    result: Optional[str] = None


# Monkey-patch the Agent class to handle client-side tools
async def _handle_model_response(
    self: Agent[None, Any],
    model_response: _messages.ModelResponse,
    run_context: RunContext[None],
    result_schema: _result.ResultSchema[Any] | None,
) -> tuple[Any | None, list[_messages.ModelRequestPart]]:
    """Process a non-streamed response from the model.

    If a client-side tool is called, return it as the final result.
    """
    tool_calls: list[_messages.ToolCallPart] = []
    for part in model_response.parts:
        if isinstance(part, _messages.ToolCallPart):
            ...
            if tool := self._function_tools.get(part.tool_name):
                if tool.client_side:
                    # Return the tool call as the final result
                    tool_request = ToolCallRequest(
                        toolCallId=part.tool_call_id or f"call_{part.tool_name}",
                        toolName=part.tool_name,
                        args=part.args,  # type: ignore
                    )
                    return _MarkFinalResult(tool_request, part.tool_name), []

    # If no client-side tools were found, proceed with normal handling
    return await original_handle_model_response(
        self, model_response, run_context, result_schema
    )


# Store the original method
original_handle_model_response = Agent._handle_model_response
# Replace with our patched version
Agent._handle_model_response = _handle_model_response  # type: ignore
