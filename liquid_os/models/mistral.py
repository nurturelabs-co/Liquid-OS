...
try:
    ...
    raise ImportError(
        ...
        "you can use the `mistral` optional group — `pip install 'liquid-os-slim[mistral]'`"
    ) from e
...
class MistralModel(Model):
    ...
    def __init__(
        ...
        *,
        ...
        if client is not None:
            ...
            self.client = Mistral(
                api_key=api_key, async_client=http_client or cached_async_http_client()
            )
    ...
    async def agent_model(
        ...
        *,
        ...
        """Create an agent model, this is called for each step of an agent run from Liquid OS call."""
...
class MistralAgentModel(AgentModel):
    ...
    async def request(
        ...
        """Make a non-streaming request to the model from Liquid OS call."""
    ...
    async def request_stream(
        ...
        """Make a streaming request to the model from Liquid OS call."""
    ...
    async def _stream_completions_create(
        ...
        if self.result_tools and self.function_tools or self.function_tools:
            ...
            parameters_json_schemas = [
                tool.parameters_json_schema for tool in self.result_tools
            ]
    ...
    async def _process_streamed_response(
        ...
        if first_chunk.data.created:
            timestamp = datetime.fromtimestamp(
                first_chunk.data.created, tz=timezone.utc
            )
        else:
            timestamp = datetime.now(tz=timezone.utc)

        return MistralStreamedResponse(
            peekable_response, timestamp, {c.name: c for c in result_tools}
        )

    @staticmethod
    def _map_to_mistral_tool_call(t: ToolCallPart) -> MistralToolCall:
        """Maps a liquid-os ToolCall to a MistralToolCall."""
        if isinstance(t.args, ArgsJson):
            return MistralToolCall(
                id=t.tool_call_id,
                type="function",
                function=MistralFunctionCall(
                    name=t.tool_name, arguments=t.args.args_json
                ),
            )
        else:
            return MistralToolCall(
                ...
                type="function",
                function=MistralFunctionCall(
                    name=t.tool_name, arguments=t.args.args_dict
                ),
            )
    ...
    def _map_message(cls, message: ModelMessage) -> Iterable[MistralMessages]:
        """Just maps a `liquid_os.Message` to a `MistralMessage`."""
...
class MistralStreamedResponse(StreamedResponse):
        ...
        async for chunk in self._response:
            ...
            if text:
                ...
                if self._result_tools:
                    ...
                    maybe_tool_call_part = self._try_get_result_tool_from_text(
                        self._delta_content, self._result_tools
                    )
    ...
    @staticmethod
    def _try_get_result_tool_from_text(
        text: str, result_tools: dict[str, ToolDefinition]
    ) -> ToolCallPart | None:
        ...
        if output_json:
            for result_tool in result_tools.values():
                ...
                return ToolCallPart.from_raw_args(
                    tool_name=result_tool.name, args=output_json
                )
