...
try:
    ...
    raise ImportError(
        ...
        "you can use the `openai` optional group — `pip install 'liquid-os-slim[openai]'`"
    ) from _import_error
...
class OpenAIAgentModel(AgentModel):
    ...
    async def request(
        ...
        try:
            ...
            console.print(f"[red]Error in OpenAI request:[/red] {str(e)}")
            console.print("[yellow]Messages that caused the error:[/yellow]")
    ...
    async def request_stream(
        ...
        try:
            ...
            console.print(f"[red]Error in OpenAI stream request:[/red] {str(e)}")
            console.print("[yellow]Messages that caused the error:[/yellow]")
            console.print(messages)
            raise
    ...
    async def _completions_create(
        ...
        try:
            ...
            console.print(f"[red]Error creating OpenAI completion:[/red] {str(e)}")
            console.print("[yellow]Request details:[/yellow]")
            console.print(f"Model: {self.model_name}")
            console.print(f"Stream: {stream}")
            console.print(f"Tool choice: {tool_choice}")
            console.print(f"Number of messages: {len(openai_messages)}")
            raise
    ...
    async def _process_streamed_response(
        ...
        return OpenAIStreamedResponse(
            peekable_response,
            datetime.fromtimestamp(first_chunk.created, tz=timezone.utc),
        )

    @classmethod
    def _map_message(
        cls, message: ModelMessage
    ) -> Iterable[chat.ChatCompletionMessageParam]:
        """Just maps a `liquid_os.Message` to a `openai.types.ChatCompletionMessageParam`."""
