...
class FunctionModel(Model):
    ...
    @overload
    def __init__(
        self, function: FunctionDef, *, stream_function: StreamFunctionDef
    ) -> None: ...
...
class AgentInfo:
    ...
    This is passed as the second to functions used within [`FunctionModel`][liquid_os.models.function.FunctionModel].
    """

    function_tools: list[ToolDefinition]
    """The function tools available on this agent.

    These are the tools registered via the [`tool`][liquid_os.Agent.tool] and
    [`tool_plain`][liquid_os.Agent.tool_plain] decorators.
    """
    allow_text_result: bool
...
FunctionDef: TypeAlias = Callable[
    [list[ModelMessage], AgentInfo], Union[ModelResponse, Awaitable[ModelResponse]]
]
"""A function used to generate a non-streamed response."""

StreamFunctionDef: TypeAlias = Callable[
    [list[ModelMessage], AgentInfo], AsyncIterator[Union[str, DeltaToolCalls]]
]
...
class FunctionAgentModel(AgentModel):
    """Implementation of `AgentModel` for [FunctionModel][liquid_os.models.function.FunctionModel]."""
    ...
    async def request(
        ...
        if inspect.iscoroutinefunction(self.function):
            ...
            response_ = await _utils.run_in_executor(
                self.function, messages, agent_info
            )
    ...
    async def request_stream(
        ...
        response_stream = PeekableAsyncStream(
            self.stream_function(messages, self.agent_info)
        )
...
class FunctionStreamedResponse(StreamedResponse):
    """Implementation of `StreamedResponse` for [FunctionModel][liquid_os.models.function.FunctionModel]."""
        ...
        async for item in self._iter:
            if isinstance(item, str):
                ...
                self._usage += usage.Usage(
                    response_tokens=response_tokens, total_tokens=response_tokens
                )
                ...
                for dtc_index, delta_tool_call in delta_tool_calls.items():
                    if delta_tool_call.json_args:
                        response_tokens = _estimate_string_tokens(
                            delta_tool_call.json_args
                        )
                        self._usage += usage.Usage(
        ...
        if isinstance(message, ModelRequest):
            ...
            for part in message.parts:
                if isinstance(part, TextPart):
                    ...
                    call = part
                    response_tokens += 1 + _estimate_string_tokens(
