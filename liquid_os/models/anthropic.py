...
try:
    ...
    raise ImportError(
        ...
        "you can use the `anthropic` optional group — `pip install 'liquid-os-slim[anthropic]'`"
    ) from _import_error
...
class AnthropicAgentModel(AgentModel):
    ...
    def _map_message(messages: list[ModelMessage]) -> tuple[str, list[MessageParam]]:
        """Just maps a `liquid_os.Message` to a `anthropic.types.MessageParam`."""
        system_prompt: str = ""
                ...
                for part in m.parts:
                    if isinstance(part, SystemPromptPart):
                        ...
                        anthropic_messages.append(
                            MessageParam(role="user", content=part.content)
                        )
                        ...
                        anthropic_messages.append(
                            MessageParam(
                                ...
                                content=[
                                    ToolResultBlockParam(
                                        tool_use_id=_guard_tool_call_id(
                                            t=part, model_source="Anthropic"
                                        ),
                                        type="tool_result",
                        ...
                        if part.tool_name is None:
                            anthropic_messages.append(
                                MessageParam(role="user", content=part.model_response())
                            )
                        else:
                            anthropic_messages.append(
                                MessageParam(
