...
from . import _result, _utils, exceptions, models
...
* may or may not take [`RunContext`][liquid_os.tools.RunContext] as a first argument
...
class _BaseRunResult(ABC, Generic[ResultData]):
    ...
    def all_messages(
        self, *, result_tool_return_content: str | None = None
    ...
    def all_messages_json(
        ...
        """Return all messages from [`all_messages`][liquid_os.result._BaseRunResult.all_messages] as JSON bytes.
    ...
    def new_messages(
        ...
        """
        return self.all_messages(result_tool_return_content=result_tool_return_content)[
    ...
    def new_messages_json(
        ...
        """Return new messages from [`new_messages`][liquid_os.result._BaseRunResult.new_messages] as JSON bytes.
...
class RunResult(_BaseRunResult[ResultData]):
    ...
    def _set_result_tool_return(
        self, return_content: str
    ) -> list[_messages.ModelMessage]:
        ...
        for part in last_message.parts:
            if (
                ...
                and part.tool_name == self._result_tool_name
            ):
                part.content = return_content
                return messages
        raise LookupError(
            f"No tool call found with tool name {self._result_tool_name!r}."
        )
...
class StreamedRunResult(_BaseRunResult[ResultData], Generic[AgentDeps, ResultData]):
    ...
    [`stream`][liquid_os.result.StreamedRunResult.stream],
    [`stream_text`][liquid_os.result.StreamedRunResult.stream_text],
    [`stream_structured`][liquid_os.result.StreamedRunResult.stream_structured] or
    [`get_data`][liquid_os.result.StreamedRunResult.get_data] completes.
    """
    ...
    async def stream(
        ...
        """
        async for structured_message, is_last in self.stream_structured(
            debounce_by=debounce_by
        ):
            result = await self.validate_structured_result(
                structured_message, allow_partial=not is_last
            )
            yield result
    ...
    async def stream_text(
        ...
        async def _stream_text_deltas() -> AsyncIterator[str]:
            async with _utils.group_by_temporal(
                _stream_text_deltas_ungrouped(), debounce_by
            ) as group_iter:
        ...
        with _logfire.span("response stream text") as lf_span:
            if delta:
                ...
                async for text in _stream_text_deltas():
                    ...
                    combined_validated_text = await self._validate_text_result(
                        combined_text
                    )
                ...
                await self._marked_completed(
                    _messages.ModelResponse.from_text(combined_validated_text)
                )
    ...
    async def stream_structured(
        ...
        with _logfire.span("response stream structured") as lf_span:
            ...
            async with _utils.group_by_temporal(
                usage_checking_stream, debounce_by
            ) as group_iter:
    ...
    async def validate_structured_result(
        ...
        if self._result_schema is not None and self._result_tool_name is not None:
            match = self._result_schema.find_named_tool(
                message.parts, self._result_tool_name
            )
            if match is None:
            ...
            result_data = result_tool.validate(
                call, allow_partial=allow_partial, wrap_validation_errors=False
            )
            ...
            text = "\n\n".join(
                x.content for x in message.parts if isinstance(x, _messages.TextPart)
            )
            for validator in self._result_validators:
